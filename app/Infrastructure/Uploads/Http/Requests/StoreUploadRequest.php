...
/**
 * Clase de solicitud para almacenar uploads.
 * 
 * Esta clase maneja la validación y autorización de solicitudes para subir archivos,
...
final class StoreUploadRequest extends FormRequest
{
    ...
    {
        // Regla personalizada para validar el profile_id
        $profileRule = function (string $attribute, mixed $value, callable $fail): void {
            try {
                // Intenta obtener el perfil para verificar su existencia
                $this->profile();
            } catch (\Throwable) {
        ...
        return array_filter([
            // ID del perfil es requerido y debe ser una cadena válida
     ...
     * Lanza una excepción de validación si el perfil no existe.
     *
     * @return UploadProfile El perfil de upload encontrado
     * @throws ValidationException Si el perfil no existe
     */
    public function profile(): UploadProfile
    {
        ...
        // Obtiene el valor del input profile_id como string
