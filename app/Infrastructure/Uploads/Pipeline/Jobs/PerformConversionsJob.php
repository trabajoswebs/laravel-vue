...
class PerformConversionsJob extends BasePerformConversionsJob
{
    ...
    public function handle(FileManipulator $fileManipulator): bool
    {
        try {
            ...
            if ($freshMedia === null) {
                $this->logger()->notice('media.conversions.skipped_missing', [
            ...
            if ($this->conversions->isEmpty()) {
                $this->logger()->info('media.conversions.no_conversions', [
            ...
            if ($currentAttempt >= $this->tries) {
                ...
                $this->logger()->alert('media.conversions.failed_permanently', [
                    'media_id'     => $this->media->getKey(),
            ...
            $this->logger()->warning('media.conversions.retrying', [
                'media_id'      => $this->media->getKey(),
                'collection'    => $this->media->collection_name ?? null,
                'attempts'      => $currentAttempt,
                'max_attempts'  => $this->tries,
    ...
    {
        $this->logger()->error('media.conversions.failed', [
            'media_id' => $this->media->getKey(),
            'collection' => $this->media->collection_name ?? null,
            'message' => $exception->getMessage(),
        ]);
    ...
    private function flushPendingCleanup(): void
    {
        try {
            ...
            $this->logger()->warning('media.conversions.cleanup_flush_failed', [
                'media_id' => $this->media->getKey(),
                'error' => $e->getMessage(),
            ]);
        }
