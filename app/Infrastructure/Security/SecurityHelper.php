...
final class SecurityHelper
{
    ...
    private const MAX_OUTPUT_SANITIZE_LENGTH = 100000;
    ...
    /**
     * Solo letras, marcas y espacios; excluye caracteres de control (Cf),
     * por lo que overrides BiDi como U+202E ya quedan bloqueados sin listas negras adicionales.
     */
     ...
     *
     * Los nombres de perfil son definidos de forma interna (por ejemplo desde el trait
     * SanitizesInputs o desde este helper) y no provienen de datos del usuario, lo cual
     * evita escenarios de "cache poisoning" seleccionando claves arbitrarias.
     *
    ...
    public static function sanitizeOutput(?string $output): string
    {
        ...
        // Limitar el tamaño antes de aplicar expresiones regulares costosas evita DoS
        // por backtracking catastrófico en payloads gigantes (ej. <script sin cerrar).
        if (mb_strlen($output, 'UTF-8') > self::MAX_OUTPUT_SANITIZE_LENGTH) {
            $output = mb_substr($output, 0, self::MAX_OUTPUT_SANITIZE_LENGTH, 'UTF-8');
        }

        // TODO: evaluar patrones alternativos menos costosos; la cota anterior mitiga su impacto.
    ...
    public static function sanitizeEmail(string $email): string
    {
        ...
        if (function_exists('idn_to_ascii') && preg_match('/[^\x20-\x7f]/u', $domain)) {
            ...
            if (str_contains($asciiDomain, 'xn--')) {
                $unicodeDomain = idn_to_utf8($asciiDomain, IDNA_DEFAULT);
                if ($unicodeDomain === false || !self::isSingleScriptDomain($unicodeDomain)) {
                    throw new \InvalidArgumentException('Dominio mezcla múltiples alfabetos');
                }
            }
    ...
    /**
     * Verifica si el dominio contiene caracteres de un único sistema de escritura (script).
     *
     * Este método ayuda a prevenir ataques de homógrafos IDN asegurando que no se mezclen
     * alfabetos (ej. Latino y Cirílico). Los caracteres comunes como puntos,
     * guiones y números son ignorados en esta validación.
     *
     * @param string $domain El nombre de dominio a validar.
     * @return bool True si el dominio usa un solo script o solo caracteres comunes; false en caso contrario.
     */
    private static function isSingleScriptDomain(string $domain): bool
    {
        ...
        $scripts = [];
        $length = mb_strlen($domain, 'UTF-8');

        for ($i = 0; $i < $length; $i++) {
            $char = mb_substr($domain, $i, 1, 'UTF-8');
            $codepoint = \IntlChar::ord($char);
            if ($codepoint === false) {
                continue;
            }

            // Ignorar puntos, guiones y dígitos
            if ($codepoint === 46 || $codepoint === 45 || ($codepoint >= 48 && $codepoint <= 57)) {
                continue;
            }

            $script = \IntlChar::getScript($codepoint);
            if (
                $script === \IntlChar::USCRIPT_COMMON ||
                $script === \IntlChar::USCRIPT_INHERITED ||
                $script === \IntlChar::USCRIPT_UNKNOWN
            ) {
                continue;
            }
     ...
     * y no use puertos no estándar. La validación de IPs se realiza en el momento
     * de sanitizar; para proteger completamente frente a DNS rebinding es
     * necesario que la capa que realiza la conexión repita estas comprobaciones
     * justo antes de abrir el socket/HTTP client.
     *
    ...
    public static function sanitizeUrl(?string $url): ?string
    {
        ...
        // Nota: sanitizar aquí garantiza IPs públicas en este instante; volver a validar antes del request real.
     ...
     *
     * Opción A (modo masked con 12 caracteres) mantiene el hash actual truncado
     * y preserva compatibilidad con logs anteriores. Opción B (modo hmac o
     * longitud extendida) aumenta la entropía y reduce el riesgo de colisiones.
     *
     * @param string|null $ip Dirección IP a hashear.
     * @return string Hash parcial de la IP y truncado según configuración.
     */
    public static function hashIp(?string $ip): string
    {
        ...
        $hashConfig = config('security.hash_ip', []);
        $length = isset($hashConfig['length']) ? (int) $hashConfig['length'] : 12;
        $length = max(8, min($length, 64)); // SHA256 tiene hasta 64 hex chars.
        $mode = $hashConfig['mode'] ?? 'masked';
        $salt = (string) config('app.key', 'default_salt');

        if ($mode === 'hmac') {
            $hmacSecret = $hashConfig['hmac_secret'] ?? $salt;
            $hash = hash_hmac('sha256', $normalizedIp, $hmacSecret);
        } else {
            $hash = hash('sha256', $normalizedIp . $salt);
        }

        // Ejemplo: 192.168.1.1 -> "a94f6d3bc12f" (modo masked, length 12)
        // Ejemplo alternativo: 192.168.1.1 -> HMAC(...) -> "5f4dcc3b5aa7"
        // Cambiar longitud o modo rompe la compatibilidad con logs previos,
        // pero mejora la resistencia ante colisiones y análisis de rainbow tables.
