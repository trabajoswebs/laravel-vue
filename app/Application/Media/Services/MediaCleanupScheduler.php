...
use App\Application\Media\DTO\CleanupStatePayload;
use App\Infrastructure\Media\Models\MediaCleanupState;
...
final class MediaCleanupScheduler
{
    ...
    public function handleConversionEvent(Media $media): void
    {
        ...
        try {
            ...
            $this->processConversionEventWithoutLock($media);
            return;
        }

        try {
            $lock->block(self::LOCK_TIMEOUT_SECONDS, function () use ($media, $mediaId): void {
                $this->processConversionEventWithoutLock($media);
            });
