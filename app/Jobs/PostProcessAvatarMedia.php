...
use App\Services\OptimizerService;                                  // Servicio de optimización basado en Spatie Image Optimizer // Ej.: $optimizer->optimize($media, 'thumb')
...
final class PostProcessAvatarMedia implements ShouldQueue, ShouldBeUnique
{
    ...
    public function __construct(
        ...
        public readonly ?int $firstSeenAtEpoch = null,  // Ej.: time()
    ...
    {
        $diskRoot = (string) Arr::get(config("filesystems.disks.{$diskName}"), 'root', '');
        if ($diskRoot === '') {
    ...
    {
        $seed = (string) ($this->firstSeenAtEpoch ?? time());
        return "ppam:releases:{$this->mediaId}:{$this->collection}:{$seed}";
    }
