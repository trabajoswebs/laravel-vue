    ...
    public function handle(): void
    {
        ...
        foreach ($this->artifacts as $disk => $directories) {
            ...
            if (!$this->isValidDisk($disk)) {
                $this->logger()->warning('cleanup_media_artifacts_invalid_disk', ['disk' => (string) $disk]);
                continue;
            }
            ...
            try {
                ...
                $this->logger()->error('cleanup_media_artifacts_disk_unavailable', [
                    'disk'  => (string) $disk,
                    'error' => $e->getMessage(),
                ]);
                continue;
            ...
            if (!$fs instanceof FilesystemAdapter) {
                $this->logger()->warning('cleanup_media_artifacts_unexpected_fs_instance', [
                    'disk'  => (string) $disk,
        ...
        $this->logger()->info('cleanup_media_artifacts_completed', [
